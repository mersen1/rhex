...
module Rhex
  class BfsPath
    ...
    def call(source, target)
      ...
      # Tie-break: neighbors equally close to the target are ordered by their distance from the
      # straight start -> target line. Both differences are the same for every neighbor, so they
      # are computed once here instead of per neighbor.
      line_delta_q = target_q - start_q
      line_delta_r = target_r - start_r
      ...
      while front < queue.size
        ...
        Constants::AXIAL_NEIGHBOR_DELTAS.each do |dq, dr|
          ...
          cross = ((line_delta_q * (start_r - nr)) - ((start_q - nq) * line_delta_r)).abs
